...
Dependencies: node/npm, python3 (+pyserial), Playwright Chromium for automated tests. `make doctor` checks and tells you what is missing.
...
| `make smoke` | Headless smoke (needs `make up`) |
...
- **Linux Chrome in WSLg** — the device appears as `/dev/ttyACM0`; try direct connect first; if Chrome cannot enumerate it, use the Windows-side option.
